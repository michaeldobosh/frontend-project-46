const plain = (data) => {
  const stringify = (object, depth = '') => {
    const result = object.children.reduce((acc, node, i, keys) => {
      ...
      if (status === 'tree') {
        ...
        acc.push(`Property '${depth}${name}' was ${status}. From ${keys[i - 1].value} to [complex value]`);
      } else if (status === 'updated' && typeof keys[i - 1].value === 'object' && value !== null) {
        ...
        acc.push(`Property '${depth}${name}' was ${status}. From ${keys[i - 1].value} to ${value}`);
      }
      return acc;
    }, []);
    return `${result.join('\n')}`;
